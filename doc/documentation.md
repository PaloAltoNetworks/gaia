...
##### `type` `emum(Default | Tenant | CloudAccount | Group | Kubernetes)` [`creation_only`]
...
##### `type` `emum(APIGateway | Docker | Host | HostService | LinuxService | WindowsService | RKT | User | SSHSession)` [`creation_only`]
...
##### `wireTagsFilteredSignature` `string`
...
##### `nativeID` `string` [`required`,`max_length=256`]
...
##### `type` `emum(Endpoint | Subnet | VPC | Interface | RouteTable | NetworkRuleSet | LoadBalancer | LoadBalancerRoute | PublicIPAddress)` [`required`]
...
##### `type` `emum(Docker | ExternalNetwork | Volume | Claim | Node | Namespace | RemoteController | APIGateway | Host | HostService | LinuxService | WindowsService | RKT | User | SSHSession)`
