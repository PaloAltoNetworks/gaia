...
##### `realm` `emum(AWSSecurityToken | AWSIdentityToken | Certificate | Google | LDAP | Vince | GCPIdentityToken | AzureIdentityToken | OIDC | SAML | AporetoIdentityToken | PCIdentityToken)` [`required`]
...
Represents a suspicious activity found on the platform.
...
Retrieves a list of suspicious activities.
...
##### `POST /suspiciousactivities`

Creates a new suspicious activity.

##### `DELETE /suspiciousactivities/:id`

Deletes the suspicious activity with the given ID.

Parameters:

- `q` (`string`): Filtering query. Consequent `q` parameters will form an or.

##### `GET /suspiciousactivities/:id`

Retrieves the suspicious activity with the given ID.

##### `PUT /suspiciousactivities/:id`

Updates the suspicious activity with the given ID.
...
##### `firstOccurrenceTime` `time` [`required`]

The timestamp when the suspicious activity first occurred.

##### `lastOccurrenceTime` `time` [`required`]
...
##### `risk` `emum(Low | Medium | High | Unknown)`
...
##### `sourceID` `string` [`required`,`creation_only`]

The identifier of the source.

##### `sourceName` `string` [`required`,`creation_only`]

The name of the source.

##### `sourceNamespace` `string` [`required`,`creation_only`]

The namespace of the source.

##### `sourceType` `string` [`required`,`creation_only`]
...
Holds the IP/FQDN of flows going to somewhere (default) external network.
...
##### `GET /unknownipaddresses`

Retrieves a list of unknown IP addresses.
...
##### `POST /unknownipaddresses`

Creates a new unknown IP address entry to store an IP/FQDN to be scanned.
...
Get reputation information of IP/FQDN from PANDB.

#### Attributes

##### `URLReputations` [`[]urlreputation`](#urlreputation) [`autogenerated`,`read_only`]
...
Represents the URL reputation information.
...
##### `categories` `[]integer` [`autogenerated`,`read_only`]

The categories codes related to the risk level.

##### `evidence` `[]string` [`autogenerated`,`read_only`]

The evidence related to the risk level.

##### `riskLevel` `string` [`autogenerated`,`read_only`]
...
##### `serviceType` `emum(Interface | Gateway | GatewayLoadBalancer | ManagedService | NotApplicable)`
...
Parameters associated with a cloud ips range.
...
##### `resourceType` `emum(Instance | Interface | Service | ProcessingUnit | PaaS)` [`required`]
...
##### `groupTags` `[]string` [`read_only`]

A list of tags to group resources.

##### `ipRanges` [`[]cloudiprange`](#cloudiprange) [`autogenerated`,`read_only`]
...
##### `storedIPRanges` [`[]cloudstorediprange`](#cloudstorediprange) [`autogenerated`,`read_only`]
...
##### `type` `emum(EffectiveSecurityGroup | SecurityGroup | ACL)` [`required`]
...
Parameters associated with a stored cloud ips range.
...
##### `state` `emum(SynTransmitted | SynAckTransmitted | AckTransmitted | SynReceived | SynAckReceived | AckProcessed | Data | Unknown | TFOProbe)` [`required`]
