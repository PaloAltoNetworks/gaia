...
Defines Next-Generation Firewall (NGFW) risk asssessment dashboard metrics.
...
```json
{
  ...
  "riskScoreConfidenceLevel": "LowConfidence",
  "riskScoreTrendLine": [
    2,
    3,
    2,
...
##### `POST /ngfwriskassessmentdashboards`

Creates a NGFW risk assessment dashboard bucket for caching stats.

##### `GET /ngfwriskassessmentdashboards/:id`

Retrieves NGFW risk assessment dashboard metrics for the specified cloud account and time filter.
...
Type: `enum(LowConfidence | MediumConfidence | HighConfidence | VeryHighConfidence)`
