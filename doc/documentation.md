...
##### `GET /bucketdestinationports`

Retrieves the list of destination port buckets.

Parameters:

- `numberOfTopEntries` (`integer`): if set, it will return the top entries up to the number specified (default 10).
...
Type: `map[string][]bucketentry`

Ports encountered and number of occurrences seen.

##### `topPorts` [`autogenerated`,`read_only`]

Type: [`[]buckettopentry`](#buckettopentry)
...
Represent a top entry within a bucket.
